...
runE2e('15. Auto-register end-to-end creates ACTIVE account', async () => {
  liveOnly('E2E_LIVE_AUTO_CREATE', 'auto-create requires live vendor credentials, live backend auth, and the operator extension connected');
  ...
  try {
    await sleep(Number(process.env.E2E_AUTO_CREATE_WAIT_MS ?? 90_000));
    ...
    assert(Boolean(created), 'no new ACTIVE VfsAccount was persisted after auto-create dispatch');
  } finally {
    controller.abort();
