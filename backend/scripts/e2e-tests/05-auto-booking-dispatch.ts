import { runE2e, skip, assert, cleanupByEmailPrefix, createTestAccount, createTestProfile, datadomeCookie, sessionCookie } from './common';

runE2e('5. Auto-booking dispatch when slot detected', async () => {
  if (process.env.E2E_LIVE_EXTENSION !== '1') {
    skip('E2E_LIVE_EXTENSION=1 is required because booking dispatch needs the operator Chrome extension connected');
  }
