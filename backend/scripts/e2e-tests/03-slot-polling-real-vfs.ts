import { runE2e, skip, assert, cleanupByEmailPrefix, createTestAccount, datadomeCookie, sessionCookie, withTestServer } from './common';

runE2e('3. Slot polling against real VFS lift-api with stored cookies', async () => {
  if (process.env.E2E_LIVE_VFS !== '1') {
    skip('E2E_LIVE_VFS=1 is required because this test must hit the real VFS lift-api with live cookies');
  }
